...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        // about each game
        newDiv.innerHTML = `<img class="game-img" src="${games[i].img}" alt="${games[i].name}"> <p>${games[i].name}</p> <p>${games[i].description}</p> <p>Backers: ${games[i].backers}</p>`;
...
document.addEventListener("DOMContentLoaded", () => addGamesToPage(GAMES_JSON));
...
const displayStr = `A total of $${totalPledged.toLocaleString("en-US")} has been raised for ${totalGames - unfundedCount} games. Currently, ${unfundedCount} ${unfundedCount == 1 ? "game" : "games"} remains unfunded. We need your help to fund these amazing games!`;
...
const firstMost = document.createElement("p");
firstMost.innerHTML = `${firstGame.name}`
firstGameContainer.append(firstMost);

// do the same for the runner up item
const secondMost = document.createElement("p");
secondMost.innerHTML = `${secondGame.name}`
secondGameContainer.append(secondMost);
...
function searchGames() {
    ...
    if (searchResults.length === 0) {
      ...
      noResultsMsg.textContent = `No games found matching "${searchInput}"`;
